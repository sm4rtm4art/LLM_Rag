#!/usr/bin/env python3
"""
Demo script for ShardedChromaVectorStore
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Demo for ShardedChromaVectorStore"
    )
    parser.add_argument(
        "--shard-capacity",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Number of documents to add in each batch"
    )
    parser.add_argument(
        "--num-batches",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="data/sharded_db",
        help="Base directory for storing shards"
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="Clean the persist directory before starting"
    )
...
def main():
    ...
    for query in search_queries:
        ...
        for i, result in enumerate(results):
            ...
            logger.info(
                f"Result {i+1}: Score={score}, ID={doc_id}, "
                f"Category={category}"
            )
        logger.info("-" * 40)
