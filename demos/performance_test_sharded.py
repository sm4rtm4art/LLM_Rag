...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Performance test for ShardedChromaVectorStore"
    )
    parser.add_argument(
        "--doc-count",
        type=int,
        default=10000,
        help="Total number of documents to test with"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="Maximum number of documents per shard"
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=4,
        help="Number of workers for concurrent search"
    )
...
def test_regular_chroma(
    ...
    for batch_idx in range(0, total_docs, batch_size):
        ...
        logger.info(f"Adding batch {batch_idx//batch_size + 1}, "
                    f"size: {batch_docs}")
        ...
        logger.info(f"Batch inserted in {batch_time:.2f} seconds, "
    ...
    for query in search_queries:
        ...
        logger.info(f"Query '{query}' completed in {query_time:.4f} seconds, "
                    f"found {len(results_found)} results")
    
    avg_search_time = total_search_time / len(search_queries)
    results["search_time"] = avg_search_time
    
    logger.info(f"Regular ChromaVectorStore results: "
...
def test_sharded_chroma(
    ...
    logger.info(f"ShardedChromaVectorStore results: "
                f"Insert: {results['insert_time']:.2f}s, "
                f"Search: {results['search_time']:.4f}s, "
                f"Throughput: {results['docs_per_second']:.2f} docs/sec, "
...
def plot_results(regular_results, sharded_results, args):
    ...
    fig.suptitle(f'ChromaVectorStore vs ShardedChromaVectorStore Performance\n'
                 f'({args.doc_count} documents, shard capacity: {args.shard_capacity})',
                 fontsize=16)
    
    # Insert time comparison
    axs[0, 0].bar(['Regular', 'Sharded'], 
    ...
    insert_speedup = regular_results['insert_time'] / sharded_results['insert_time'] if sharded_results['insert_time'] > 0 else 0
    search_speedup = regular_results['search_time'] / sharded_results['search_time'] if sharded_results['search_time'] > 0 else 0
    throughput_speedup = sharded_results['docs_per_second'] / regular_results['docs_per_second'] if regular_results['docs_per_second'] > 0 else 0
