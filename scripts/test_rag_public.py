...
class MockLLM(LLM):
    ...
    def _call(
        ...
        **kwargs: Any,
        ...
        return (
            f"This is a mock response to the question: '{question}'. "
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Test the RAG pipeline with public test data."
    )
    parser.add_argument(
        "--query",
        type=str,
        default="What is RAG?",
        help="The query to test with the RAG pipeline."
    )
    parser.add_argument(
        "--collection",
        type=str,
        default="public_test",
        help="The name of the collection in the vector store."
    )
    parser.add_argument(
        "--db-path",
        type=str,
        default="data/vectorstore",
        help="The path to the vector store database."
    )
    parser.add_argument(
        "--top-k",
        type=int,
...
def main():
    ...
    if args.use_test_query is not None:
        ...
        if test_queries and 0 <= args.use_test_query < len(test_queries):
            ...
            logger.info(
                "Using test query %d: %s",
                args.use_test_query,
                query
            )
        else:
            logger.warning(
                "Invalid test query index: %d. Using default query.",
                args.use_test_query
            )
