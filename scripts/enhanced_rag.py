...
def is_purpose_query(query: str) -> bool:
    ...
    """
    purpose_keywords = [
        "zweck", "purpose", "ziel", "goal", 
        "anwendungsbereich", "scope", "application area",
        "zitiere", "quote", "vollständig", "complete",
        "satz", "sentence", "beginnt", "starts"
    ]
    ...
    if doc_id:
        ...
        if is_purpose_query(query):
            ...
            if purpose_statement:
                ...
                response = (
                    f"Der vollständige Zweck-Satz aus dem Dokument lautet:"
                    f"\n\n\"{purpose_statement}\""
                )
        ...
        if relevant_docs:
            confidence = sum(
                score for _, score in relevant_docs
            ) / len(relevant_docs)
        else:
            confidence = 0.0
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced RAG with improved retrieval and prompting"
    )
    parser.add_argument(
        "--query",
        type=str,
