...
class TestShardedChromaVectorStore(unittest.TestCase):
    ...
    def test_initialization(self):
        ...
        with patch.object(ChromaVectorStore, '__init__', return_value=None) as mock_init:
            store = ShardedChromaVectorStore(
                shard_capacity=5000,
                base_persist_directory=self.temp_dir,
                max_workers=2
            )
    ...
    def test_add_documents_within_capacity(self):
        ...
        with patch.object(ChromaVectorStore, '__init__', return_value=None):
            ...
            mock_shard.count.return_value = 0
    ...
    def test_add_documents_exceeding_capacity(self):
        ...
        with patch.object(ChromaVectorStore, '__init__', return_value=None):
            mock_shard = MagicMock()
            mock_shard.count.return_value = 95
            mock_shard.add_documents = MagicMock()
    ...
    def test_search_aggregates_results(self):
        ...
        mock_shard1.search.return_value = [
            {"content": "doc1", "score": 0.9, "metadata": {"source": "shard1"}}
        ]
        
        mock_shard2 = MagicMock()
        mock_shard2.search.return_value = [
            {"content": "doc2", "score": 0.8, "metadata": {"source": "shard2"}}
        ]
        ...
        with patch.object(ChromaVectorStore, '__init__', return_value=None):
            ...
            self.assertEqual(results[0]["content"], "doc1")  # Higher score (0.9)
            self.assertEqual(results[1]["content"], "doc2")  # Lower score (0.8)
    ...
    def test_search_limits_results(self):
        ...
        with patch.object(ChromaVectorStore, '__init__', return_value=None):
            ...
            self.assertEqual(results[0]["content"], "doc1")  # 0.9
            self.assertEqual(results[1]["content"], "doc2")  # 0.8
            self.assertEqual(results[2]["content"], "doc3")  # 0.7
    
    def test_count_aggregates_from_shards(self):
        """Test that count aggregates document counts from all shards."""
        # Arrange
        mock_shard1 = MagicMock()
        mock_shard1.count.return_value = 100
        
        mock_shard2 = MagicMock()
        mock_shard2.count.return_value = 200
